...
def sweep(interpreters: list[str], freeze: str | None) -> int:
    ...
    # (The pin does not set PYTHONNOUSERSITE/PYTHONHOME, so a resolved interpreter still
    # activates its own user site — all children share that exposure equally.)
    resolved: list[str] = []
    ...
    # Two names aliasing to one binary would shrink the sweep while the summary still
    # claimed len(interpreters) members — refuse rather than overclaim.
    by_real_path: dict[str, str] = {}
    for name, path in zip(interpreters, resolved):
        real = os.path.realpath(path)
        if real in by_real_path:
            print(
                f"DUPLICATE interpreter: {name!r} and {by_real_path[real]!r} "
                f"both resolve to {real}"
            )
            return 2
        by_real_path[real] = name
