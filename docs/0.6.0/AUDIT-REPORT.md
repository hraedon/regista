...
- **The review gate's *logic* is sound** where it isn't bypassed: the deliberate human-proxy loosening admits no hidden mind (verified across six delegation shapes); separation of duties holds through delegation; multi-pass histories downgrade correctly; a reviewer who authors after its own pass is caught; `request_changes` carried the same acknowledgment obligation at audit time; `principal_kind` ingress validation is real and canonicalising; 296 gate tests pass. **Post-audit amendment (2026-08-13, WI-284):** `request_changes` now requires a substantive finding but deliberately carries no independence obligation because it makes no positive assurance claim. Canonical v3 and persisted canonical v1/v2 behavior are both pinned through transition tests.
