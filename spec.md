...
- Transparency-log anchoring of the event log to an external append-only log — ~~implemented (FR-30)~~ **REMOVED by P1.4** (0.6.0): the subsystem never anchored a single receipt estate-wide and is replaced by the bundle v3 signed statement (`docs/0.6.0/BUNDLE-V3.md`)
...
- FR-03 **[MVP]**: Append events to immutable per-project event log. Each event carries:
  ...
  - `on_behalf_of` (jsonb, optional) — delegation indicator for agent actors acting under a human principal. Contains `principal_id` (required, string), optional `scope` (list of strings), and optional `authenticated_at` (ISO-8601 string). Trust tier: *actor-claimed*; tamper protection is HMAC over the field, but authorization is self-attested by the caller. See Plan 010.
  ...
  - `prev_event_hash` — SHA-256 of `(canonical_envelope ∥ signature)` of the preceding event in the same work-item's event sequence. `None` for the first event (event_seq=1). Provides tamper-proof ordering within a work-item's event chain (BC-233). Replay verifies the chain and increments `warnings` on break.
  - `global_seq` — `BIGSERIAL` monotonic across the entire project. Enables coherent batch timestamping (Plan 014) and cross-work-item event ordering.
...
- FR-10: Flag `needs_review` and emit `escalated` event when `attempt_number` ≥ workflow-declared threshold. Idempotency: unique partial index `(work_item_id) WHERE transition = 'escalated'`; second insert raises and is silently dropped.
...
- FR-16 **[MVP]**: Replay — rebuild a `work_items_current_replay_<timestamp>` projection from the event log on demand. Each historical transition validates against the workflow version recorded on its event. regista does NOT mutate live `work_items_current` in place.

  The replay report is returned in-band on the :class:`ReplayReport` object via its ``entries`` field — one :class:`ReplayReportEntry` per work-item, each carrying ``work_item_id``, ``category``, ``detail``, and ``warnings``. This is the portable access path and is populated in both normal and read-only modes; comparison logic does not need to be authored per-replay. The report categories are:
  ...
  Nonzero ``replayed_drift`` count is a defect signal. ``halted`` rows are operator alerts; live projection is untouched on halt. Nonzero ``warnings`` count with zero ``halted`` and zero ``drift`` indicates a clean replay with historical key rotation.

  Under the hood the writable backend also materializes temporary ``work_items_current_replay_<tag>`` and ``replay_report_<tag>`` tables during the replay transaction (``report.table_name``); these are connection-scoped and dropped when replay returns, so they are NOT a valid post-return handle — use ``report.entries`` instead. This keeps replay read-only with respect to the store: it leaves no permanent residue (previously it created permanent tables) and can run against a read-only connection (``read_only=True``), where results are held entirely in memory and ``report.table_name`` is ``None``.

  > **REMOVED by P1.4 (0.6.0):** `replay(verify_timestamps=True)` and the `tsp_batches` table were deleted with the RFC 3161 timestamping subsystem (zero batches existed estate-wide; migration 045 drops the table). Replay no longer takes a `verify_timestamps` parameter.
...
- **FR-28**: Recurring work-item generation (Plan 003). Register a recurrence rule that creates work-items on a schedule. Supports interval (`P1D`, `PT1H`) or RRULE schedules. Rules live in the new `recurrence_rules` table. Public API: `register_recurrence_rule`, `list_recurrence_rules`, `due_recurrences`, `fire_recurrence`, `update_recurrence_rule`, `cancel_recurrence_rule`.
  ...
  - **Interval schedule note**: ISO-8601 durations containing months (`P1M`) or years (`P1Y`) are converted to `timedelta` using a fixed baseline (1970-01-01), which yields 31 days for `P1M` and 365 days for `P1Y`. This is an approximation; true calendar-month or calendar-year scheduling requires RRULE.
...
- **FR-30**: ~~Transparency-log anchoring (Plan 019)~~ — **REMOVED by P1.4 (0.6.0)**: `_anchoring.py`, the `anchor_receipts` table (dropped by migration 045), the `anchoring` facade, and the `regista anchor` CLI verbs were deleted; zero anchor receipts existed estate-wide, and external verifiability is re-established by the bundle v3 signed statement plus published trust roots (`docs/0.6.0/BUNDLE-V3.md`). Historical description follows. Transparency-log anchoring (Plan 019). Periodically publishes a content-committing anchor of the event log to an external append-only log via a pluggable `AnchorProvider`, so any post-hoc modification of covered events is externally detectable without trusting the operator. The anchor (`compute_content_anchor`, migration 042) is `sha256(chain_head_hash + project_name + target_global_seq + envelope_version + hash_algorithm)`, where `chain_head_hash = sha256(canonical_envelope + signature)` of the last event in the batch — committing to event *content* (not event UUIDs), transitively covering all preceding events via the `prev_global_event_hash` chain. Three providers: `OpenTimestampsProvider` (Bitcoin-anchored, async confirmation; optional dep `pip install regista[anchoring]`), `RFC3161AnchorProvider` (synchronous trusted TSA), `FileAnchorProvider` (operator-controlled append-only file; **not** externally verifiable — see §17.9.2). New `anchor_receipts` table (migrations 041 + 042); maintenance thread submits on `anchor_interval` and upgrades pending OTS proofs on `anchor_upgrade_interval`. Receipt state machine: `pending → confirmed` (success path) or `pending → retryable → confirmed|failed` (failure path), with advisory-lock serialization and `INSERT ... ON CONFLICT` for concurrent-submission safety. Public API: `trigger_anchoring`, `upgrade_pending_anchors`, `retry_failed_anchors`, `list_anchor_receipts`, `get_anchor_receipt`, `verify_anchor_receipt`, plus the `anchoring` facade. CLI: `regista anchor submit|status|verify`. New error codes: `ANCHOR_PROVIDER_UNAVAILABLE`, `ANCHOR_SUBMISSION_FAILED`, `ANCHOR_RECEIPT_NOT_FOUND`, `ANCHOR_VERIFY_FAILED`. Trust-model implications documented in §17.9.2; known residual risk tracked as BC-300 (OTS verify pending ambiguity).
...
| Signing envelope | Decided | Pluggable `SigningScheme` (Plan 011): HMAC-SHA256 (default) or Ed25519. Key declares scheme in `KeyEntry.scheme`; `scheme_id` persisted on each event row. Envelope v3: RFC 8785 (JCS) canonical JSON of `{event_id, work_item_id, actor_id, key_id, event_seq, workflow_name, workflow_version, timestamp, on_behalf_of, transition, payload, prev_event_hash?, global_seq?}`. Keys with `None` values omitted. Backward compat: replay classifies envelope version (v3→v2→v1) at verification time. `prev_event_hash` (BC-233) chains each event to its predecessor: `SHA-256(prev_canonical_envelope ∥ prev_signature)`, stored as `prev_event_hash BYTEA`. `global_seq BIGSERIAL` enables coherent cross-WI timestamping (Plan 014). |
...
- AC-17 [FR-16]: `replay()` produces a `work_items_current_replay_<tag>` projection (connection-scoped temp table, dropped on return) and a `replay_report_<tag>` table during the transaction; live `work_items_current` is unchanged. The durable, portable results are on ``report.entries``. Each historical transition is validated against the workflow version recorded on its event. Encountering a revoked-key event halts replay on that work-item with operator alert.
...
- AC-27 [FR-16]: ``report.entries`` contains exactly one :class:`ReplayReportEntry` per work-item processed, categorized as ``replayed_ok``, ``replayed_drift``, or ``halted`` (with per-entry ``warnings``). Drift count of zero is the success criterion for a no-bug projection.
...
|---|---|---|
| Authenticated | every field the event's envelope version signs — for v5: `event_id`, `entity_kind`, `entity_id`, `actor_id`, `actor_kind`, `actor_metadata`, `key_id`, `event_seq`, `workflow_name`, `workflow_version`, `timestamp`, `hash_alg`, `on_behalf_of`, `transition`, `payload`, and the chain links `prev_event_hash` / `prev_global_event_hash` when present | Scheme-verified (HMAC-SHA256 or Ed25519) over the stored `canonical_envelope`, **and** the row column reconciled against those signed bytes (WI-267). Tampering with either the envelope or the row is detected at re-verification. |
| Alias-bound | `work_item_id` | Not itself signed from envelope v4 onward (`entity_id` is the signed identifier, migration 031), but required to equal the signed `entity_id`, so it cannot steer a consumer to a different entity than the signature covers. |
| Unsigned | `global_seq`, `scheme_id` | In **no** envelope version. `global_seq` is assigned post-signing (§17.11) and was backfilled by a `(timestamp, event_id)` proxy in migration 017; `scheme_id` was stamped `'hmac-sha256'` onto every pre-015 row. Ordering must be established by walking `prev_global_event_hash`, and the signing scheme must be taken from trusted key metadata — never from the row. |
| Actor-claimed | The *contents* of `actor_metadata` (`role`, `model`, `provider`, `role_source`, `context_hash`, `prompt_template_hash`) and of `actor_kind` | Self-asserted by the actor. From envelope **v5** these fields are inside the signing envelope and reconciled against the row, so it is non-repudiable that *this actor said this* and a database-write attacker cannot change it — but the claim itself is still the actor's. When FR-24 actor roles are registered, `role` is additionally enforced against the actor's registered set; otherwise treated as diagnostic. |

**Envelope version matters.** `actor_kind` and `actor_metadata` are signed only from **v5** (WI-208). A v4 event's values for those two fields are unauthenticated *forever* — no signature covers them and none can be added without rewriting history. Verification reports such an event as `legacy_partial` and names the unsigned fields; a consumer that makes a trust decision from a field listed in `unsigned_fields` is reading an unauthenticated value. (Earlier revisions of this section said `actor_kind`/`actor_metadata` are "not included in the canonical signing envelope". That was true of v4 and is false of v5.)
...
**Verification is one primitive.** `regista._verification.verify_event_strict` is the only function that decides whether an event is authenticated. It verifies the exact stored `canonical_envelope` bytes and then requires every field that envelope version signs to agree with its row column, returning a structured `VerificationResult` (envelope version and schema validity, signature validity, trusted-key source, row reconciliation and the *names* of any mismatched fields, authenticated vs unsigned field sets, and a final applicability of `fully_authenticated` / `legacy_partial` / `invalid` / `unverifiable`). There is no policy, flag or environment variable that turns a signed-field mismatch into a pass, and no fallback to an envelope rebuilt from the row once a stored envelope exists.
...
- **Residual risk under both schemes:** The operator (or anyone who obtains the signing key) can sign arbitrary events and insert them into the log. Asymmetric signing prevents *third-party* forgery but does not prevent *operator* forgery. Mitigations: the event hash chain (BC-233) makes retroactive tampering detectable (inserting or reordering events breaks the chain). The transparency-log anchor that previously claimed external detectability here was **REMOVED by P1.4 (0.6.0)** — it never anchored a receipt estate-wide; operator forgery is addressed in 0.6.0 by the signed bundle v3 statement verified against externally published trust roots (`docs/0.6.0/BUNDLE-V3.md`, `docs/0.6.0/OPERATOR-FORGERY.md`).

### 17.9.2 Transparency-log anchoring posture

> **REMOVED by P1.4 (0.6.0):** everything this section describes — `AnchorProvider`, the three providers, `anchor_receipts`, `regista anchor verify` — was deleted. Zero anchor receipts existed estate-wide. The operator-forgery posture for 0.6.0 is defined in `docs/0.6.0/OPERATOR-FORGERY.md` and `docs/0.6.0/BUNDLE-V3.md`. The text below is retained as historical record only.

Plan 019 (FR-30) closes the residual operator-forgery gap named above. The maintenance thread periodically computes a content-committing anchor over the event log and submits it to an external append-only log via a pluggable `AnchorProvider`. The anchor is `sha256(chain_head_hash + project_name + target_global_seq + envelope_version + hash_algorithm)` (migration 042), where `chain_head_hash = sha256(canonical_envelope + signature)` of the last event in the batch — this commits to actual event content (not event UUIDs), transitively covering all preceding events via the `prev_global_event_hash` chain links. Three providers ship:

- **OpenTimestampsProvider** — anchors the root to Bitcoin via the public OpenTimestamps network. Confirmation is asynchronous (Bitcoin block latency, ~hours). This is the canonical public-anchor story: any third party with the receipt and a Bitcoin node can verify that a root existed at a specific block height, without trusting the operator. Optional dependency (`pip install regista[anchoring]`).
- **RFC3161AnchorProvider** — wraps a trusted Time-Stamping Authority under the same `AnchorProvider` interface. Synchronous; the TSA vouches for the root's existence at a signed time. Trust in the TSA is assumed.
- **FileAnchorProvider** — appends the root to an operator-controlled append-only location (local file; in real deployments, an S3 object-lock target or equivalent WORM store). **Not a cryptographic external anchor** — the operator who can rewrite the event log can also rewrite this file. It provides a second tamper-evident copy on a (presumably separate) operator-controlled medium, suitable as a no-network escape hatch for air-gapped regulated deployments. For external verifiability against a backdating operator, use OpenTimestamps or RFC3161.

**Anchor receipts** (`anchor_receipts` table, migrations 041 + 042) persist each submission with `status ∈ {pending, committed, confirmed, failed, retryable}`, the provider-specific receipt blob (OTS proof, RFC 3161 token, or file log line), the `target_global_seq` the anchor covers, and the binding fields (`project_name`, `envelope_version`, `hash_algorithm`). The `retryable` state (migration 042) allows transient failures to be retried without creating duplicate receipts. Concurrency safety: an advisory lock serializes batch selection, and `INSERT ... ON CONFLICT DO NOTHING` with `SELECT ... FOR UPDATE` handles duplicate roots, upgrading `retryable` receipts in place when a better status arrives. The maintenance thread submits new batches on `anchor_interval` and upgrades pending OTS proofs toward Bitcoin confirmation on `anchor_upgrade_interval`.

**Verification** (`regista anchor verify <receipt_id>`): performs two checks. First, `verify_content_anchor` recomputes the anchor from live events through `target_global_seq`, validating each `prev_global_event_hash` chain link and each `payload_canonical_hash` against the stored `canonical_envelope` — detecting any mutation of event content, signature, actor, or chain hash. Second, the provider's `verify(merkle_root, receipt)` confirms the external proof (Bitcoin attestation for OTS, TSA token for RFC 3161, file entry for File). Returns `confirmed` (both checks pass), `pending` (OTS proof not yet upgraded to a Bitcoin attestation), or `failed` (either the live-event recomputation or the external proof check fails). Tamper tests verify that mutating any security-relevant event field produces a `failed` result.

**Residual risk after anchoring:**

- **The pre-anchor window.** Events appended after the last confirmed anchor are defended only by HMAC/Ed25519 (§17.9.1). The window is bounded by `anchor_interval` (default 1h) plus provider latency (Bitcoin confirmation adds ~hours for OTS). Pre-anchor events are *not yet* externally anchored.
- **Per-receipt verification recomputes from live events.** The `anchor verify` command recomputes the content anchor from live events through `target_global_seq` and validates chain links, then checks the external proof. A backdating operator who rewrites any field in the canonical signing envelope (content, signature, actor_id, chain hash) within an already-anchored batch is detected. However, the following mutations are **not** detected by the anchor: (a) a mutation of the denormalised `payload` jsonb column *without* touching `canonical_envelope` — the anchor commits to the envelope, not to the row, so it cannot see this; since WI-267 row↔envelope reconciliation during replay and every other verification path does catch it (before WI-267 nothing did, because verification returned as soon as the stored envelope verified); (b) for **v4 and earlier** envelopes only, a mutation of `actor_kind` or `actor_metadata`, which those versions do not include in the canonical signing envelope and which are therefore not covered by the chain head hash or the anchor. From **v5** (WI-208) both fields are inside the signed envelope and, since WI-267, are reconciled against the row on every verification, so such a mutation is detected by replay and by every other verification path. Deletion of an entire event (rather than mutation) is detected by the chain-link check, but deletion of the *last* event in a batch leaves the anchor covering the remaining events — deletion of pre-anchor events is not externally detectable until the next anchor.
- **OTS verify without a Bitcoin node.** `OpenTimestampsProvider.verify` returns `pending` both for proofs awaiting Bitcoin attestation and for proofs the verifier cannot reach a Bitcoin node to confirm. The latter case can mask a tamper signal until the verifier runs with node connectivity (BC-300).
- **FileAnchorProvider** is operator-trusted, not externally verifiable, as noted above.

The trust-model claim that survives this section: *events covered by a confirmed external anchor cannot be modified post-hoc without detection by any third party who holds the receipt and has access to the anchor's verification path (a Bitcoin node for OTS, the TSA's published certificate for RFC3161).* This is the differentiator versus a system that ships only signing.
...
**Witness key enrollment — WITHDRAWN in 0.6.0 (WI-238, WI-264).** This section previously said an Ed25519 witness's public key was "enrolled into the anchored `principal_keys` registry", so downstream verifiers "can treat the witness key as a trust root pinned in the same registry as actor principals". **That sentence was false and is withdrawn.** The registry was never anchored: enrollment was a plain `INSERT` into `principal_keys` with no event emitted anywhere, so there was nothing for a verifier to pin to (`docs/0.6.0/TRUST-DOMAIN.md` §7.1). Correcting it is required whether or not witness work ever lands, because it is exactly the class of claim 0.6.0 exists to stop making.

What is true in 0.6.0:

- `principal_keys` is a **projection** of signed trust-log lifecycle events, never a trust root and never consulted for a v6 event (`TRUST-DOMAIN.md` §5.9). Its rows are rebuildable with `regista trust rebuild-projection`.
- **Witness lifecycle is cut from this release** (`TRUST-DOMAIN.md` §7 CUT marker, D-7). There are zero witness registrations and zero receipts estate-wide, so nothing is migrated and no deployed evidence is lost. Registering an Ed25519 witness, rotating its key, and unregistering a witness that still holds registry rows now **refuse** with `WITNESS_LIFECYCLE_CUT` rather than performing an unsourced registry write.
- Webhook delivery is retained as **non-evidentiary transport** only (`key_scheme="hmac-sha256"`). No security claim attaches to it.
- `enrolled_witness_key(witness_id)` and the `witness:key_enrollment:<project>` doctor check remain as **read-only reporting** over whatever rows a store already contains. A warn from that check means the store disagrees with itself; it is not a statement that any key is anchored.

A signed witness lifecycle and a genuinely external witness belong in a later release that has an actual witness.
