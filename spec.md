...
- **Per-receipt verification recomputes from live events.** The `anchor verify` command recomputes the content anchor from live events through `target_global_seq` and validates chain links, then checks the external proof. A backdating operator who rewrites any field in the canonical signing envelope (content, signature, actor_id, chain hash) within an already-anchored batch is detected. However, the following mutations are **not** detected by the anchor: (a) a mutation of the denormalised `payload` jsonb column *without* touching `canonical_envelope` (signature verification during replay would catch this); (b) a mutation of `actor_kind` or `actor_metadata`, which are stored on the event row but **not included** in the canonical signing envelope and therefore not covered by the chain head hash or the anchor (replay also cannot detect this — see §17.9 actor-claimed trust tier). Deletion of an entire event (rather than mutation) is detected by the chain-link check, but deletion of the *last* event in a batch leaves the anchor covering the remaining events — deletion of pre-anchor events is not externally detectable until the next anchor.
