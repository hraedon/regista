...
**Witness key enrollment in the anchored registry (WI-238).** An Ed25519 witness's public key is also enrolled into the anchored `principal_keys` registry under the deterministic principal id `witness:<witness_id>`, so downstream verifiers (e.g. cairn) can treat the witness key as a trust root pinned in the same registry as actor principals, rather than trusting the unverified `witness_registrations.public_key` column in isolation. Enrollment shares the registration transaction. The lifecycle mirrors principal-key enrollment: registration enrolls (`registered_by="witness-enrollment"`); `rotate_witness_key` supersedes the old key and activates the new one atomically (also updating `witness_registrations.public_key`); `unregister_witness` revokes the active enrolled key (it remains in the registry, revoked, for historical verification). `enrolled_witness_key(witness_id)` returns the active anchored entry; the key is also reachable via the principal-keys facade (`principals.get_active("witness:<witness_id>")`). `regista doctor` surfaces a per-project `witness:key_enrollment:<project>` check that warns when an Ed25519 witness's pinned key is absent from, or disagrees with, the registry.
