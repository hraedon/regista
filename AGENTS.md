...
- **Plan 025 WI-4.1 (Version surface):** `_version_info.py` with `versions()` returning `VersionInfo` (frozen dataclass): `library_version`, `schema_version` (47), `canonical_workflow_version`, `envelope_version` (5), `canonical_workflow_hash`, `available_signing_schemes`. CLI: `regista version --json`. API: `regista.versions()`. Schema version is the highest migration number; envelope version is the signed-envelope format version. `docs/suite-config.md` documents the shape a `SUITE.lock` records.
